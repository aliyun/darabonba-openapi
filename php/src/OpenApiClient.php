...
class OpenApiClient
{
  ...
  protected $_enableUsageDataCollection;
  ...
  public function doRPCRequest($action, $version, $protocol, $method, $authType, $bodyType, $request, $runtime)
  {
    ...
    while (Tea::allowRetry(@$_runtime["retry"], $_retryTimes, $_now)) {
      ...
      try {
        ...
        if (!Utils::isUnset($this->_enableUsageDataCollection) && $this->_enableUsageDataCollection) {
          $hostname = Utils::getHostName();
          if (Utils::isUnset($hostname)) {
            $hostname = "unknown";
          }
    ...
    while (Tea::allowRetry(@$_runtime["retry"], $_retryTimes, $_now)) {
      ...
      try {
        ...
        if (Utils::is4xx($_response->statusCode) || Utils::is5xx($_response->statusCode)) {
          ...
          $requestId = self::defaultAny($requestId, @$err["request_id"]);
          $errorCode = self::defaultAny(@$err["Code"], @$err["code"]);
          $errorCode = self::defaultAny($errorCode, @$err["error"]);
          $errorMessage = self::defaultAny(@$err["Message"], @$err["message"]);
          $errorMessage = self::defaultAny($errorMessage, @$err["error_description"]);
          @$err["statusCode"] = $_response->statusCode;
          throw new TeaError([
            "code" => "" . (string) ($errorCode) . "",
            "message" => "code: " . (string) ($_response->statusCode) . ", " . (string) ($errorMessage) . " request id: " . (string) ($requestId) . "",
            "data" => $err,
            "description" => "" . (string) (self::defaultAny(@$err["Description"], @$err["description"])) . "",
            "accessDeniedDetail" => self::defaultAny(@$err["AccessDeniedDetail"], @$err["accessDeniedDetail"])
          ]);
