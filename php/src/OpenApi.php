...
use AlibabaCloud\OpenApiUtil\OpenApi as AlibabaCloudOpenApiUtilOpenApi;
        ...
        while (Tea::allowRetry($_runtime['retry'], $_retryTimes, $_now)) {
            ...
            try {
                ...
                $_request->query    = Tea::merge([
                    ...
                    'Format'         => 'json',
                    'Version'        => $version,
                    'Timestamp'      => AlibabaCloudOpenApiUtilOpenApi::getTimestamp(),
                ...
                if (!Utils::isUnset($request->body)) {
                    $tmp                               = Utils::anyifyMapValue(AlibabaCloudOpenApiUtilOpenApi::query($request->body));
                ...
                if (!Utils::equalString($authType, 'Anonymous')) {
                    ...
                    $signedParam                         = Tea::merge($_request->query, AlibabaCloudOpenApiUtilOpenApi::query($request->body));
                    $_request->query['Signature']        = AlibabaCloudOpenApiUtilOpenApi::getRPCSignature($signedParam, $_request->method, $accessKeySecret);
                }
        ...
        while (Tea::allowRetry($_runtime['retry'], $_retryTimes, $_now)) {
            ...
            try {
                ...
                if (!Utils::equalString($authType, 'Anonymous')) {
                    ...
                    $stringToSign                       = AlibabaCloudOpenApiUtilOpenApi::getStringToSign($_request);
                    $_request->headers['authorization'] = 'acs ' . $accessKeyId . ':' . AlibabaCloudOpenApiUtilOpenApi::getROASignature($stringToSign, $accessKeySecret) . '';
                }
                $_lastRequest = $_request;
                $_response    = Tea::send($_request, $_runtime);
    ...
    public function doROARequestWithForm($action, $version, $protocol, $method, $authType, $pathname, $bodyType, $request, $runtime)
    {
        ...
        while (Tea::allowRetry($_runtime['retry'], $_retryTimes, $_now)) {
            ...
            try {
                ...
                if (!Utils::isUnset($request->body)) {
                    $_request->body                    = AlibabaCloudOpenApiUtilOpenApi::toForm($request->body);
                    $_request->headers['content-type'] = 'application/x-www-form-urlencoded';
                }
                if (!Utils::isUnset($request->query)) {
                    $_request->query = $request->query;
