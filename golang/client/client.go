...
func (client *Client) Init(config *Config) (_err error) {
	if tea.BoolValue(util.IsUnset(config)) {
		_err = &ClientError{
			Code:    tea.String("ParameterMissing"),
			Message: tea.String("'config' can not be unset"),
		}
		return _err
	}
...
func (client *Client) DoRPCRequest(action *string, version *string, protocol *string, method *string, authType *string, bodyType *string, request *OpenApiRequest, runtime *util.RuntimeOptions) (_result map[string]interface{}, _err error) {
	...
	_runtime := tea.NewRuntimeObject(map[string]interface{}{
	...
	var retryPolicyContext *tea.RetryPolicyContext
	var request_ *tea.Request
	var response_ *tea.Response
	retriesAttempted := int(0)
	retryPolicyContext = &tea.RetryPolicyContext{
		RetriesAttempted: tea.Int(retriesAttempted),
	}
	_resp := make(map[string]interface{})
	for tea.BoolValue(tea.ShouldRetry(_runtime.RetryOptions, retryPolicyContext)) {
		_backoffDelayTime := tea.GetBackoffDelay(_runtime.RetryOptions, retryPolicyContext)
		tea.Sleep(_backoffDelayTime)
		...
		_resp, _err = func() (map[string]interface{}, error) {
			...
			if tea.BoolValue(util.Is4xx(response_.StatusCode)) || tea.BoolValue(util.Is5xx(response_.StatusCode)) {
				...
				if tea.BoolValue(util.EqualString(tea.String(tea.ToString(code)), tea.String("Throttling"))) || tea.BoolValue(util.EqualString(tea.String(tea.ToString(code)), tea.String("Throttling.User"))) || tea.BoolValue(util.EqualString(tea.String(tea.ToString(code)), tea.String("Throttling.Api"))) {
					_err = &ThrottlingError{
						StatusCode:  response_.StatusCode,
						Code:        tea.String(tea.ToString(code)),
						Message:     tea.String("code: " + tea.ToString(tea.IntValue(response_.StatusCode)) + ", " + tea.ToString(DefaultAny(err["Message"], err["message"])) + " request id: " + tea.ToString(requestId)),
						Description: tea.String(tea.ToString(DefaultAny(err["Description"], err["description"]))),
						RetryAfter:  openapiutil.GetThrottlingTimeLeft(response_.Headers),
						RequestId:   tea.String(tea.ToString(requestId)),
					}
					return _result, _err
				} else if tea.BoolValue(util.Is4xx(response_.StatusCode)) {
					_err = &ClientError{
						StatusCode:         response_.StatusCode,
						Code:               tea.String(tea.ToString(code)),
						Message:            tea.String("code: " + tea.ToString(tea.IntValue(response_.StatusCode)) + ", " + tea.ToString(DefaultAny(err["Message"], err["message"])) + " request id: " + tea.ToString(requestId)),
...
func (client *Client) CallApi(params *Params, request *OpenApiRequest, runtime *util.RuntimeOptions) (_result map[string]interface{}, _err error) {
	if tea.BoolValue(util.IsUnset(params)) {
		_err = &ClientError{
			Code:    tea.String("ParameterMissing"),
			Message: tea.String("'params' can not be unset"),
		}
		return _result, _err
	}
...
func (client *Client) CheckConfig(config *Config) (_err error) {
	if tea.BoolValue(util.Empty(client.EndpointRule)) && tea.BoolValue(util.Empty(config.Endpoint)) {
		_err = &ClientError{
			Code:    tea.String("ParameterMissing"),
			Message: tea.String("'config.endpoint' can not be unset"),
		}
		return _err
	}
...
func (client *Client) GetAccessDeniedDetail(err map[string]interface{}) (_result map[string]interface{}) {
	...
	if tea.BoolValue(util.IsUnset(err["AccessDeniedDetail"])) {
		detail1, _err := util.AssertAsMap(err["AccessDeniedDetail"])
		if _err != nil {
			return _result
		}

		accessDeniedDetail = detail1
	} else if tea.BoolValue(util.IsUnset(err["accessDeniedDetail"])) {
		detail2, _err := util.AssertAsMap(err["AccessDeniedDetail"])
		if _err != nil {
			return _result
		}
